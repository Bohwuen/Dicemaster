...
function Me.FormatDiceString( dice, modifier )
	...
	if Me.PermittedUse() and mod > 20 then
		mod = 20
	end
...
local function PrintDiceMasterRoll( name, count, sides, mod, rolls, rollType, broadcast )
	...
	local is_self = myName.."-"..myRealm == name	
...
function Me.Roll( dice, rollType, unitName )
	...
	if type( dice ) ~= "string" then 
		return UIError( "Formato de dados inválido." )
	end
	
	local count, sides, modtype, mod = dice:match("^%s*(%d*)[dD](%d+)([+-]?)(%d*)%s*$")
	
	if not count then return UIError( "Formato de dados inválido." ) end
