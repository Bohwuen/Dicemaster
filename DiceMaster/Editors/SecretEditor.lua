...
local function TriggerSecret( secret, secretIndex, traitIndex )
	...
	if not( DiceMasterPanel.traits[traitIndex].secret[secretButton]:IsShown() ) then
		return
	end
	...
	DiceMasterPanel.traits[traitIndex].secret[secretButton]:Hide()
...
function Me.SecretEditor_EnableSecret( traitIndex )
	...
	DiceMasterPanel.traits[ traitIndex ].secret:Show()
...
function Me.SecretEditorCondition_OnLoad(frame, level, menuList)
	...
	for i = 1, #Me.Profile.stats do
		if not Me.Profile.stats[i].value then
			statsList[ Me.Profile.stats[i].name ] = {}
			...
			tinsert( statsList[ lastCategory ], Me.Profile.stats[i] )
		end
	end
	...
	elseif menuList == "Statistics" then
	...
	elseif menuList then
		for i = 1,#statsList[menuList] do
			...
			if statsList[menuList][i].stat then
				info.tooltipText = statsList[menuList][i].desc .. "|n|cFF707070(Modified by "..statsList[menuList][i].stat.." + "..info.text..")|r";
			end
...
function Me.SecretEditor_DeleteSecret()
	...
	DiceMasterPanel.traits[Me.editing_trait].secret:Hide()
...
function Me.SecretEditor_Save()
	if Me.SecretEditor.secretDesc1.EditBox:GetText() == "" then
		UIErrorsFrame:AddMessage( "Invalid description: too short.", 1.0, 0.0, 0.0, 53, 5 );
		return
	end
	...
	and ( not Me.SecretEditor.conditionsList3 or next(Me.SecretEditor.conditionsList3) == nil ) then
		UIErrorsFrame:AddMessage( "You must choose at least one condition for a secret.", 1.0, 0.0, 0.0, 53, 5 );
		return
	end
