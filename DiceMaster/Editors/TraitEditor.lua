...
StaticPopupDialogs["DICEMASTER4_IMPORTITEM"] = {
  ...
  OnAccept = function (self, data)
	...
	if T and type( T ) == "table" and T.name and T.stackCount and T.stackSize and T.guid then
		...
		UIErrorsFrame:AddMessage( "Corrupt item data found. Unable to import code.", 1.0, 0.0, 0.0, 53, 5 );
	end
  end,
  hasEditBox = true,
  timeout = 0,
...
local INVENTORY_TUTORIAL = {
	"You can export an item with a unique import code which other players can use to import the item.|n|nUse the search box to search for specific items based on keyword.|n|nClick the Clean Up Inventory button to clean up your bags. It auto-sorts and moves items out of the way to make room for new items.",
	"Custom items occupy a slot in your inventory, up to a maximum of 42 slots.",
	"You can create, edit, copy, or sell items from your inventory by using these buttons.|n|nDungeon Masters can use the Loot button to allow group members to roll on an item in their inventory.",
...
function Me.TraitEditor_LootOnLoad(frame, level, menuList)
	...
	info.tooltipText = "Dispense an item to a specific player."
	UIDropDownMenu_AddButton(info, level);
