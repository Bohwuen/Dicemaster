...
local WAITING_FOR_TRAIT = {
	...
	name    = "";
	serial  = 0; 
	desc    = "Esperando datos del jugador.";
	approved = 0;
	secret1Active = false;
...
local function PrimeInspectData( name )
	...
	Me.inspectData[name] = {
		...
		charges = {
			...
			count     = 0;
			max       = 0;
			tooltip   = "Representa la cantidad de Cargas que ha acumulado para ciertos rasgos.";
			symbol    = "charge-orb";
			flash     = true;
		};
		health        = 5;
	...
	if status then
		...
		if Me.IsLeader( true ) then
			DiceMaster4.SetupTooltip( DiceMasterInspectFrame.health, nil, "Salud", nil, nil, nil, nil, 
              "Representa la salud del personaje.|n|cFF707070<Click Izquierdo/Derecho para Añadir/Quitar>|n<Shift+Click Izquierdo para fijar Máximo>|n<Ctrl+Click izquierdo para Fijar valor>" )
		else
...
function Me.Inspect_OnExperience( data, dist, sender )
	...
	if data.v then
		...
		Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t Experiencia adquirida: " .. ( data.v ) .. ".", "RAID")
		
		if Profile.experience >= 100 then
			Profile.experience = Profile.experience - 100
			Profile.level = Profile.level + 1;
			PlaySound(124)
			Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t ¡Enhorabuena, has alcanzado el nivel " .. ( Profile.level ) .. "!", "RAID")
		end
	elseif data.l then
		Profile.level = Profile.level + 1;
		PlaySound(124)
	...
	elseif data.r then	
		...
		Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t Tu nivel ha sido restablecido a 1.", "RAID")
	end
